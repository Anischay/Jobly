        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
          ...
          {steps.map((step, index) => (
            <motion.div
              ...
              className="group bg-gray-800/50 backdrop-blur-sm rounded-xl p-8 shadow-xl hover:shadow-2xl transition-shadow min-h-[320px] flex flex-col relative z-10 border border-purple-400/20"
            >
              <div className="flex-1">
                <motion.div
                  whileHover={{ rotate: 360 }}
                  transition={{ duration: 0.5 }}
                  className="w-16 h-16 rounded-full bg-purple-500/10 flex items-center justify-center mb-6 group-hover:bg-purple-500/20 transition-colors"
                >
                  {step.icon}
                </motion.div>
                <motion.h3
                  whileHover={{ x: 5 }}
                  className="text-2xl font-semibold text-white mb-4 bg-gradient-to-r from-purple-400 to-blue-400 bg-clip-text text-transparent"
                >
                  {step.title}
                </motion.h3>
              ...
              <motion.div
                whileHover={{ x: 5 }}
                className="flex items-center gap-2 text-purple-400 font-medium mt-6 opacity-0 group-hover:opacity-100 transition-opacity"
              >
                Learn more
